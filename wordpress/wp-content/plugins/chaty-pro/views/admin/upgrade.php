<div class="upgrade-wrap">
    <a href="<?php echo esc_url(admin_url('admin.php?page=chaty-app')) ?>" class="link-back"><?php esc_attr_e('Back', CHT_PRO_OPT); ?></a>
            ...
            <div class="wrap wrap-licenses">
                ...
                if (isset($m) && !empty($m)) {
                    switch ($m) {
                        case "error":
                            $class_name = "error";
                            $message = esc_attr__("Your license key is not valid", CHT_PRO_OPT);
                            break;
                        case "valid":
                            $class_name = "success";
                            $message = esc_attr__("Your license key is activated successfully", CHT_PRO_OPT);
                            break;
                        case "unactivated":
                            $class_name = "success";
                            $message = esc_attr__("Your license key is deactivated successfully", CHT_PRO_OPT);
                            break;
                        case "expired":
                            $class_name = "error";
                            $message = esc_attr__("Your license has been expired", CHT_PRO_OPT);
                            break;
                        case "invalid":
                            $class_name = "error";
                            $message = esc_attr__("Your request is not valid", CHT_PRO_OPT);
                            break;
                        case "no_activations":
                            $class_name = "error";
                            $message = esc_attr__("Your request is not valid", CHT_PRO_OPT);
                ...
                <form action="" method="post" id="license_action_form">
                    ...
                    <div class="license-key">
                        <div class="license-key-header">
                            <?php esc_attr_e("License Key", CHT_PRO_OPT) ?>
                        </div>
                        <div class="license-key-content">
                            ...
                            <div class="license-key-message">
                                <?php if ($isLicenseActive) { ?>
                                    <button type="submit" class="button-secondary remove-testimonial-license-key"><?php esc_attr_e("Deactivate License", CHT_PRO_OPT) ?></button>
                                <?php } ?>
                                <input type="hidden" name="action" value="activate_deactivate_chaty_license_key">
                                <input type="hidden" id="license_action_type" name="action_type" value="">
                            </div>
                        </div>
                        <div class="license-key-footer">
                            <?php
                            if (!$isLicenseActive) {
                                esc_attr_e("To receive updates, please enter your valid Software Licensing license key.", CHT_PRO_OPT);
                    ...
                    <button type="submit" id="submit" class="button button-primary save-testimonial-license-key"><?php esc_attr_e("Save Changes", CHT_PRO_OPT) ?></button>
                    <?php if ($isLicenseActive == 2) { ?>
                        <a target="_blank" href="<?php echo esc_url(CHT_CHATY_PLUGIN_URL."/checkout/?edd_license_key=".$licenseKey."&download_id=".CHT_CHATY_PLUGIN_ID) ?>" class="button button-primary renew-form-btn"><?php esc_attr_e("Renew Now", CHT_PRO_OPT) ?></a>
                    <?php } ?>
...
<script>
    jQuery(document).ready(function(){
        ...
        jQuery(".save-testimonial-license-key").click(function(){
            jQuery("#license_action_type").val("save");
            jQuery(this).attr("disabled",true);
            submitChatyLicenceForm();
        });
        jQuery(".remove-testimonial-license-key").click(function(){
